// Battle.js - No 'let game = window.game' here
var game; // Defined globally via index.html
...
function startEffectCheck(user) {
    ...
    try {
        ...
        if (user.statusEffects.some(function(e) { return e.name === "Numb" && e.startOfTurnFunction; })) {
            console.log("[DEBUG]: Processing Numb for", user.name);
            user.statusEffects.find(function(e) { return e.name === "Numb"; }).startOfTurnFunction(user, game.target, game.battleScene);
            user.statusEffects = user.statusEffects.filter(function(e) { return e.name !== "Numb"; });
...
function skillAction(user) {
    ...
    var isSupportSkill = skillSet.findSkill(skill.name).support || false;
...
function triggeredEffectCheck(user, target, skillStyle) {
    try {
        target.statusEffects.forEach(function(effect) {
            if (effect.triggered && effect.triggeredFunction) {
                var endSkill = effect.triggeredFunction(user, target, game.battleScene, skillStyle);
                if (endSkill) return;
            }
        });
    } catch (e) {
    ...
    if (game.target.hp <= 0) {
        if (game.battleType === "training") {
            ...
            if (game.player.exp === game.player.maxExp) {
                performJutsuSelection(1, false, function() { ArriveVillage(game.player.lastVillage); });
            } else {
...
function updateStatus() {
    try {
        ...
        if (playerHp && enemyHp && game.user && game.target) {
            ...
            if (log) {
                log.innerHTML += "<p>Status - " + game.user.name + ": HP " + game.user.hp + "/" + game.user.maxHp + ", Effects: " + game.user.statusEffects.map(function(e) { return e.name; }).join(", ") + "</p>";
                log.innerHTML += "<p>Status - " + game.target.name + ": HP " + game.target.hp + "/" + game.target.maxHp + ", Effects: " + game.target.statusEffects.map(function(e) { return e.name; }).join(", ") + "</p>";
            }
        }
    } catch (e) {
        console.error("[ERROR]: Update status failed:", e);
