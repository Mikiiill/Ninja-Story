...
function takeTurn(name) {
    ...
    setTimeout(() => {
        ...
        let skill = user.skills[Math.floor(Math.random() * user.skills.length)]; // Draw a skill
        if (user.statusEffects.some(e => e.name === "Numb")) {
            ...
            if (target.statusEffects.some(e => e.name === "Substitution") && !skill.support) {
            ...
            } else if (target.statusEffects.some(e => e.name === "ShadowCloneEffect") && !skill.support) {
...
function endBattle() {
    ...
    // Transition to village after training fight
    setTimeout(() => {
        game.gameState = "In Village";
        queueOutput("Arrived at Newb Village! HP restored, status effects cleared.");
