    ...
    async substitutionJutsu(user, target) {
        user.statusEffects.push(new StatusEffect("Swap", 1, 0, false, false, true, null, null, 
            async (target, user, skillStyle) => {
                if (!inBattle) return false; // NEW: Check inBattle
...
async function awardReward(winner, loser) {
    if (!inBattle && game.battleType === "training" && (winner === player || (winner === "both" && loser === "both"))) {
        player.xp += 1;
...
async function checkForDeath() {
    if (!inBattle || !game.player || !game.target) return false;
...
async function takeTurn() {
    ...
    try {
        ...
        updateBattleUI();
        if (!inBattle || checkForDeath()) return;
        if (skipTurn) {
...
async function skillAction() {
    try {
        if (!inBattle) {
            logBattle("Cannot perform action: Battle is not active!");
            await sleep(3000);
            return;
        }
